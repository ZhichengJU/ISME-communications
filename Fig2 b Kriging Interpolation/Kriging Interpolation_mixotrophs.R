...
# 加载location.txt和mix_alpha.txt数据
location_data <- read.table("location.txt", header = TRUE)
mix_Autumn <- read.table("mix_Autumn.txt", header = TRUE)
# 读取香港地图文件
hk <- st_read("hk_map.geojson")
# 将location_data和mix_Autumn合并成一个数据框
data1 <- merge(mix_Autumn,location_data, by = "Sample")
...
#   xlab(NULL) +
#   ylab(NULL)
# ggsave("fig/mix_site.png", device = "png", width = 8, height = 6, dpi = 900)
...
# 颜色会变浅，不好看
# ggsave('fig/mix_alpha_map.pdf', p, height = 6, width = 8, dpi = 900)  # 保存图片并设置宽和高
# 边上会有阴影
ggsave("mix/mix_Autumn_map.pdf", device = cairo_pdf, width = 8, height = 6)
# 保存为 PNG 文件
ggsave("mix/mix_Autumn_map.png", device = "png", width = 8, height = 6, dpi = 900)
