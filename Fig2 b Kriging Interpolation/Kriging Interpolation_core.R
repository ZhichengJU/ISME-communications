...
# 加载location.txt和core_alpha.txt数据
location_data <- read.table("location.txt", header = TRUE)
core_Summer <- read.table("core_Summer.txt", header = TRUE)
# 读取香港地图文件
hk <- st_read("hk_map.geojson")
# 将location_data和core_Summer合并成一个数据框
data1 <- merge(core_Summer,location_data, by = "Sample")
...
#   xlab(NULL) +
#   ylab(NULL)
# ggsave("fig/core_site.png", device = "png", width = 8, height = 6, dpi = 900)
...
p <- ggplot() +
  ...
  scale_fill_gradientn(colors = my_colormap, limits = c(0, 4.7)) +  # 保留填充颜色的图例
  geom_point(data = data1, aes(x = lon, y = lat, color = shannon), size = 3, shape = 16) +
  scale_color_gradientn(colors = my_colormap, limits = c(0, 4.7)) +  # 删除这行代码，去除点的颜色图例
...
# 颜色会变浅，不好看
# ggsave('fig/core_alpha_map.pdf', p, height = 6, width = 8, dpi = 900)  # 保存图片并设置宽和高
# 边上会有阴影
ggsave("core/core_Summer_map.pdf", device = cairo_pdf, width = 8, height = 6)
# 保存为 PNG 文件
ggsave("core/core_Summer_map.png", device = "png", width = 8, height = 6, dpi = 900)
